...
{#if media.title && media.releaseDate}
	<div
		class="mt-0 flex animate-fade-in items-center justify-start rounded-md py-1 hover:bg-background-dark md:py-2"
	>
		<DialogClose>
			<a
				href={resolve(
			...
			>
				...
				<div class="flex flex-col items-baseline text-left">
					<p class="text-[15px] font-medium">
						{media.title} ({media.releaseDate.toString().slice(0, 4)})
					</p>
					<p
